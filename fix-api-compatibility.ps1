...
if (!(Test-Path $FLLAMA_DIR)) {
    ...
    Write-Host "Please run the update script first to create your fork" -ForegroundColor Yellow
    exit 1
}
...
# Fix 1: Remove flash_attn lines (lines 686-687)
$content = $content -replace 'ctx_params\.flash_attn = false;\s*', ''
$content = $content -replace 'log_message\("\[fllama\] flash_attn: " \+ std::to_string\(ctx_params\.flash_attn\), request\.dart_logger\);\s*', ''
...
# Fix 2: Update common_chat_format_example call (line 968)
$content = Get-Content $fllamaFile -Raw
...
- Replace deprecated token functions:
...
Write-Host "  ✓ Pushed to https://github.com/dragneel2074/fllama" -ForegroundColor Green
